		...
		public function admin_init() {
			...
			add_settings_field(
				...
				array(
					...
					'description' => __( sprintf( 'What to show as Company identifier by default for shortcodes if not provided shortcode parameter %s. Widget have own setting for this.', "'show'" ), 'wpaust' ),
					'items'       => array(
			...
			add_settings_field(
				...
				array(
					...
					'description' => __(
						'Custom template for item. You can use macro keywords %exch_symbol%, %symbol%, %company%, %price%, %change% and %changep% mixed with HTML tags &lt;span&gt;, &lt;em&gt; and/or &lt;strong&gt;.',
						'wpaust'
					),
					'class' => 'widefat',
					'rows'  => 2,
		...
		public function stock_ticker_sanitize($input) {
			...
			if ( ! @file_put_contents( $upload_dir['basedir'] . '/stock-ticker-custom.css', $css, LOCK_EX ) ) {
				...
				add_settings_error(
					...
					sprintf(
						__( 'Failed to write custom CSS file because of error <em>%s</em><br />Please create mentioned file manually and add following code to it:<br /><code>%s</code>', 'stock-ticker' ),
						$error['message'],
						$css
					),
					'error'
			...
			if ( ! empty( $input['refresh'] ) ) {
				...
				if ( ! @file_put_contents( $upload_dir['basedir'] . '/stock-ticker-refresh.js', $js, LOCK_EX ) ) {
					...
					add_settings_error(
						...
						sprintf(
							__( 'Failed to write custom JS file for auto refreshing ticker because of error <em>%s</em><br />Please create mentioned file manually and add following code to it:<br /><code>%s</code>', 'stock-ticker' ),
							$error['message'],
							$js
						),
						'error'
